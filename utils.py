...
def calcular_deudas(diccionario):
    if not diccionario:
        ...
        mensaje.append(f"\nTotal gastado por el grupo: ${total_gastado:.2f}") 
        print(f"\nPromedio de gasto por persona: ${promedio:.2f}")
        mensaje.append(f"\nPromedio de gasto por persona: ${promedio:.2f}") 
        ...
        for persona, total in totales.items():
            ...
            mensaje.append(f"{persona} ha puesto un total de: ${total:.2f}") 
        ...
        while deudores and acreedores:
            ...
            if deuda > credito:
                ...
                mensaje.append(f"{deudor} le debe a {acreedor} ${credito:.2f}") 
                ...
                mensaje.append(f"{deudor} le debe a {acreedor} ${deuda:.2f}")
...
def exportar_txt(nombre_grupo, mensaje):
    with open(f'{nombre_grupo}_deudas.txt', 'w') as archivo:
